...
#define WIFI_SSID "Church Mifi"
#define WIFI_PASSWORD "christ77"

//For the following credentials, see examples/Authentications/SignInAsUser/EmailPassword/EmailPassword.ino

/* 2. Define the API Key */
#define API_KEY "AIzaSyBEG7ULATYhY93C96psSS19HB3N56gM4As"

/* 3. Define the RTDB URL */
#define DATABASE_URL "vetappointmentbooker-default-rtdb.firebaseio.com" //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
...
void loop()
{
 ...
 h = Serial.read();
  ...
  FirebaseJson json1;
  FirebaseJson json;
    json1.set("1/" + String("Status"), F("1"));
      Serial.printf("Set json... %s\n", Firebase.set(fbdo, F("/parkings"), json1) ? "ok" : fbdo.errorReason().c_str());
    
    if (h == 0)
    {
         json1.set("1/" + String("ParkingAvailability"), F("available"));
        json1.set("1/" + String("ParkingNumber"), F("1"));
      Serial.printf("Set json... %s\n", Firebase.set(fbdo, F("/parkings"), json1) ? "ok" : fbdo.errorReason().c_str());
     json.set("2/" + String("ParkingAvailability"), F("available"));
        json.set("2/" + String("ParkingNumber"), F("2"));
      Serial.printf("Set json... %s\n", Firebase.set(fbdo, F("/parkings"), json) ? "ok" : fbdo.errorReason().c_str());
   }
    else if(h==1){
        json1.set("1/" + String("ParkingAvailability"), F("taken"));
        json1.set("1/" + String("ParkingNumber"), F("1"));
      Serial.printf("Set json... %s\n", Firebase.set(fbdo, F("/parkings"), json1) ? "ok" : fbdo.errorReason().c_str());
    
    }
     else if(h==2){
        json.set("2/" + String("ParkingAvailability"), F("taken"));
        json.set("2/" + String("ParkingNumber"), F("2"));
      Serial.printf("Set json... %s\n", Firebase.set(fbdo, F("/parkings"), json) ? "ok" : fbdo.errorReason().c_str());
    }
   delay(1000);
